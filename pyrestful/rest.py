...
def config(func,method,**kwparams):
	...
	path     = None   # 路由
	produces = None   # 指定返回的内容类型，仅当request请求头中的(Accept)类型中包含该指定类型才返回
	consumes = None   # 指定处理请求的提交内容类型（Content-Type），例如application/json, text/html;
	types    = None   # 参数数据类型
	async    = False  # 是否异步
	...
	if len(kwparams):
		...
		# 判断异步还是同步
		if '_async' in kwparams:
			async = kwparams['_async']
	...
	operation._async           = async
	
	if operation._produces not in [mediatypes.APPLICATION_JSON,mediatypes.APPLICATION_XML,mediatypes.TEXT_XML, None]:
		raise PyRestfulException("The media type used do not exist : " + operation.func_name)  # 请求头中不包含响应的内容类型

	return operation  # 返回一次http请求的相关信息
...
class RestHandler(tornado.web.RequestHandler):
	"""
	rest base handler,业务逻辑handler继承该 hanlder
	"""
	...
	def _exe(self, method):
		...
		# 获取该handler定义的所有的method(get,post...)
		...
		for operation in list(map(lambda op: getattr(self,op), functions)):
			...
			async                  = getattr(operation, "_async")
			...
			if operation._method == self.request.method and service_name == services_from_request and len(service_params) + len(service_name) == len(services_and_params):
				try:
					...
					if consumes == mediatypes.APPLICATION_XML:
						...
						p_values.append(param_obj)
					if async:
						response = yield operation(self, *p_values)
					else:
...
class RestService(tornado.web.Application):
	...
	def __init__(self, rest_handlers, resource=None, handlers=None, default_host="", transforms=None, **settings):
		...
		for r in rest_handlers:
			svs = self._generateRestServices(r)  # 组装路由与handler的映射
			restservices += svs
